...
/// How many addresses may wait for a path-open retry at once.
///
/// **LOCAL PATCH — NOT UPSTREAM IROH.** See `vendor/iroh/DIASWARM-PATCH.md`.
const MAX_PENDING_OPEN_PATHS: usize = 64;

/// Queue `addr` for a later path-open retry, deduplicated and capped.
///
/// Upstream pushes unconditionally. The retry then drains the queue and calls
/// `open_path_on_all_conns` for each entry, which re-attempts the address on
/// *every* connection to that peer — and every connection still at its path-id
/// cap re-queues the same address. So one entry becomes C entries per 333 ms
/// tick, and the `VecDeque` grows geometrically until the allocator gives up.
///
/// This is verbatim the hot-fix posted by cbenhagen in iroh#4390.
fn enqueue_pending_open_path(
    queue: &mut VecDeque<transports::FourTuple>,
    addr: transports::FourTuple,
) {
    if queue.contains(&addr) {
        return; // already pending -> already going to be retried
    }
            ...
            None => {
                ...
                match ret {
                    ...
                    | Some(Err(PathError::MaxPathIdReached)) => {
                        ...
                        // PATCHED LOCALLY — see vendor/iroh/DIASWARM-PATCH.md and
