...
/// AVAILABILITY, WITHOUT ANYONE TYPING A SECOND ADDRESS.
///
/// The relay case was demonstrable long before it was real: a reader fetched
/// from a peer while the subject was off the air, but only because the test —
/// and, on real hardware, a person — handed it that peer's address. A follower
/// in the field knows exactly one address, the one it scanned, so the moment
/// that device sleeps there is nowhere else to ask and replicating to a swarm
/// buys nothing.
///
/// Here nobody types anything. A relay replicates the subject; a reader scans
/// only the SUBJECT's invite, discovers the relay through it, and then survives
/// the subject going away.
#[tokio::test(flavor = "multi_thread")]
async fn a_follower_discovers_another_peer_and_survives_the_subject_leaving() {
    let reader = Identity::generate();

    let subject_store = tmp("disc-subject");
    let subject = subject_with_history(&subject_store, &reader);
    let subject_hex = hex(&subject.enc_public());
    let subject_router = serve_with(subject_store.clone(), SecretKey::generate(), true).await.unwrap();
    let subject_addr = upstream(&subject_router.endpoint().addr());

    // --- a relay replicates, which is how the subject learns it exists ---
    //
    // It serves FIRST and then fetches through its own endpoint, so the id the
    // subject records is the id the relay can be dialled at. Fetching from a
    // throwaway endpoint — which is what happens if you do this the obvious way
    // round — records an address that stops existing when the sync ends.
    let relay_store = tmp("disc-relay");
    let relay_router = serve_with(relay_store.clone(), SecretKey::generate(), true).await.unwrap();
    let relay_id = relay_router.endpoint().id().to_string();
    add_follow(&relay_store, &subject_hex, &subject_addr, None).unwrap();
    assert!(
        diaswarm_net::peer::refresh_all_on(relay_router.endpoint(), &relay_store)
            .await.unwrap()[0].reached()
    );

    // The relay must itself have fetched, so the subject recorded it.
    // Stored as `id@addr,addr` — the id authenticates, the addresses are how it
    // is reached without a discovery service.
    let known = diaswarm_net::holders(&subject_store.join(&subject_hex));
    assert!(
        known.iter().any(|h| h.split('@').next() == Some(relay_id.as_str())),
        "the subject did not notice the relay replicating it: {known:?}"
    );

    // --- the reader scans ONLY the subject's invite ---
    let reader_store = tmp("disc-reader");
    add_follow(&reader_store, &subject_hex, &subject_addr, Some("follow")).unwrap();
    let first = refresh_all(&reader_store, true).await.unwrap();
    assert!(first[0].reached());

    // It should now know about the relay without anybody saying so.
    let follows = diaswarm_net::peer::load_follows(&reader_store).unwrap();
    assert!(
        follows[0].from.iter().any(|f| f.split('@').next() == Some(relay_id.as_str())),
        "the reader learned no second address: {:?}",
        follows[0].from
    );
    ...
    // A fresh day exists only on the relay now, so reaching it proves the
    // reader used the address it discovered rather than the one it scanned.
    let relay_vault = Vault::open(&relay_store.join(&subject_hex)).unwrap();
    let held_before = relay_vault.segments().unwrap().len();

    let second = refresh_all(&reader_store, true).await.unwrap();
    assert!(
        second[0].reached(),
        "the follower died with the subject — discovery bought nothing: {:?}",
        second[0].failures
    );
    assert_eq!(
        second[0].via.as_deref().and_then(|v| v.split('@').next()),
        Some(relay_id.as_str()),
        "reached, but not via the relay"
    );

    let opened = Vault::open(&reader_store.join(&subject_hex)).unwrap().read_as(&reader, "follow").unwrap();
    assert_eq!(opened.len(), 3, "the reader could not open what the relay served");
    assert_eq!(held_before, 3);

    relay_router.shutdown().await.unwrap();
}

/// A peer cannot put somebody else's address in your list.
///
/// The id recorded is the authenticated end of the QUIC connection, never a
/// value out of the request. Otherwise discovery would be a way to aim
/// followers at an address of an attacker's choosing.
#[test]
fn a_holder_list_only_ever_records_the_speaker() {
    let dir = tmp("holders");
    diaswarm_net::remember_holder(&dir, &"aa".repeat(32)).unwrap();
    diaswarm_net::remember_holder(&dir, "not-an-endpoint").unwrap();
    diaswarm_net::remember_holder(&dir, &"aa".repeat(32)).unwrap();
